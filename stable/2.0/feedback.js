...
(function($){
	...
	if (supportedBrowser) {
		if(isFeedbackButtonNative) {
			//$(settings.mainElementSelector).append('<button class="feedback-btn feedback-btn-gray">' + settings.initButtonText + '</button>');
		}
		...
		var init = function(){
			...
			$(settings.mainElementSelector).append(tpl);
			...
			$(document).on('mouseup', function(){
				if (canDraw) {
					...
					var $fbHelper = $('<div class="feedback-helper" data-type="' + dtype + '" data-time="' + Date.now() + '" style="position:absolute;top:' + dtop + 'px;left:' + dleft + 'px;width:' + dwidth + 'px;height:' + dheight + 'px;z-index:30000;"></div>');
					$('#feedback-helpers').append($fbHelper);
			...
			if (settings.highlightElement) {
				...
				$(document).on('mousemove click', '#feedback-canvas',function(e) {
					if (canDraw) {
						...
						$('* :not(settings.mainElementSelector,script,iframe,div,section,.feedback-btn,#feedback-module *)').each(function(){
						...
						if ($toHighlight && !drag) {
							...
							if (e.type == 'click' && e.pageX == rect.startX && e.pageY == rect.startY) {
								...
								$.feedback.addHelper(hidx, _x, _y, _w, _h);
								++hidx;
							}
						}
					}
			...
			$(document).on('mouseleave', 'settings.mainElementSelector,#feedback-canvas', function() {
				redraw(ctx);
			});
			...
			$(document).on('click', '#feedback-close', $.feedback.removeHelper);
			...
			$(document).on('click', '#feedback-highlighter-next', function() {
				...
				html2canvas($(settings.mainElementSelector), {
					onrendered: function(canvas) {
						if (!settings.screenshotStroke) {
							redraw(ctx);
						}
						_canvas = $('<canvas id="feedback-canvas-tmp" width="'+ w +'" height="'+ dh +'"/>').hide().appendTo(settings.mainElementSelector);
		...
		$.feedback.init = init;
		$.feedback.addHelper = function(hidx, _x, _y, _w, _h) {
			...
			$(document).trigger("onAddHelper", [hidx]);

			redraw(ctx);

		};
		$.feedback.removeHelper = function(e) {
			
			//e can be event fromCloseClick or hidx
			var fromCloseClick = e.target != undefined && e.currentTarget != undefined;
			
			var $target = fromCloseClick?  $(this).parent() : $('[data-highlight-id="' + e + '"]') ;

			
			if (settings.highlightElement && $target.attr('data-highlight-id'))
				var _hidx = fromCloseClick ? $target.attr('data-highlight-id') : e;
			...
			$(document).trigger("onRemoveHelper", [_hidx]);
	...
	function close() {
		...
		$(document).off('mouseleave', settings.mainElementSelector);
